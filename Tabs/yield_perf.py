...
st.markdown(f"# 🌱 Yield Performa (Ton/ha)")
...
st.markdown("### 🏢 Ringkasan Performa Tingkat Kebun")
c_y_b, c_y_y = st.columns(2)

with c_y_b:
    st.subheader(f"Yield Kebun - Per Bulan {pilihan_bulan}")
    fig_k_bln = go.Figure()
...
with c_y_y:
    st.subheader(f"Yield Kebun - Akumulasi YTD")
    fig_k_ytd = go.Figure()
...
with c_fig_afd_b:
    st.subheader(f"Yield Afdeling - Bulan {pilihan_bulan}")
    fig_a_b = go.Figure()
...
with c_fig_afd_y:
    st.subheader(f"Yield Afdeling - Akumulasi YTD")
    fig_a_y = go.Figure()
