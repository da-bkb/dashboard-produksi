...
def load_data(tipe_target):
    ...
    # 1. Baca File CSV dengan pendeteksian separator otomatis
    if os.path.exists(file_name):
        ...
        st.error(f"File {file_name} tidak ditemukan di direktori utama!")
    ...
    for col in kolom_numerik:
        if col in df.columns:
            # Saring: Hanya bersihkan jika kolom masih dibaca sebagai Teks/Object
            if df[col].dtype == 'object':
                # Ganti koma desimal menjadi titik desimal
                df[col] = df[col].astype(str).str.replace(',', '.', regex=False)
            
            # Ubah paksa menjadi numerik desimal/float
...
if not df_raw.empty:
    ...
    st.warning("Gagal memuat data. Silakan periksa kembali file CSV Anda.")