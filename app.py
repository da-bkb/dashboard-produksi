...
# --- PROSES LOADING DATA DENGAN CLEANING AMAN ---
@st.cache_data
def load_data(tipe_target):
    ...
    # Pastikan file ada sebelum dibaca untuk menghindari crash putus jalur
    if not os.path.exists(file_name):
        st.error(f"⚠️ File data `{file_name}` tidak ditemukan di root server! Pastikan nama file sudah benar.")
        return pd.DataFrame(), nama_target

    # Pembacaan menggunakan sep; karena data menggunakan format titik koma
    try:
    ...
    # Standarisasi kolom Bulan (Hapus spasi & jadikan huruf besar semua)
    if 'Bulan' in df.columns:
        df['Bulan'] = df['Bulan'].astype(str).str.strip().str.upper()
        
    # KOREKSI DATA STR / OBJECT MENJADI NUMERIK DESIMAL
    kolom_angka = [
    ...
    for col in kolom_angka:
        if col in df.columns:
            if df[col].dtype == 'object':
                # Ubah koma desimal Indonesia menjadi titik desimal standar python
...
# --- JALANKAN DASHBOARD JIKA DATA TIDAK KOSONG ---
if not df_raw.empty:
    ...
    # Jaga-jaga jika ada bulan lain yang tidak masuk daftar standar
    ...
    # Kirim parameter bulan ke session state global
    ...
    # PERUBAHAN URUTAN TAB SESUAI PERMINTAAN BAPAK:
    if nama_target == "Budget":
        ...
        # Urutan Tab Khusus Sensus (Yield -> J/P -> BJR -> Trend Kebun)
    ...
    st.error("❌ Memerlukan file database. Pastikan Rekap26.csv atau Rekap26_Sns.csv sudah di-upload ke repositori GitHub Bapak.")