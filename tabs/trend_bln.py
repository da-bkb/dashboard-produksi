...
# Proteksi janjang pokok/RJP jika data pokok tidak tersedia (luas ha * 135 pokok/ha jika tidak ada)
...
# 3. Suntik Garis Hubung Putus-Putus
tambah_garis_hubung(fig_yield, df_trend, "Yield Akt.", "Yield Bgt.")

# Menampilkan anotasi angka persentase dengan perbaikan parameter font (weight="bold")
for idx, row in df_trend.iterrows():
    if row["Yield Akt."] > 0:
        fig_yield.add_annotation(
            ...
            font=dict(color="#28348A", size=10, weight="bold")  # Perbaikan di sini
        )
...
# Suntik Garis Hubung Putus-Putus
