...
st.markdown(f"### 🌱 RJP terhadap Budget (Janjang/Pokok)")
...
col_bgt = next((c for c in cols if ('bgt' in c.lower() or 'budget' in c.lower()) and any(x in c.lower() for x in ['jg', 'jjg', 'jan', 'janjang'])), None)

# Proteksi jika kolom tidak ditemukan sama sekali
if not col_akt or not col_bgt:
    st.error(f"❌ **Error: Kolom Janjang/Budget tidak terdeteksi secara otomatis!**")
    st.markdown(f"""
    Aplikasi tidak dapat menemukan kombinasi kolom Janjang Aktual / Budget di file Anda.
    * Kolom Aktual ditemukan: `{col_akt}`
    * Kolom Budget ditemukan: `{col_bgt}`
...
with col_g1:
    st.markdown(f"##### 📊 RJP Per Kebun - {pilihan_bulan}")
    fig_mtd = go.Figure()
    ...
    st.plotly_chart(fig_mtd, use_container_width=True, key="chart_rjp_bgt_kebun_mtd")

with col_g2:
    st.markdown(f"##### 📊 RJP Per Kebun - s.d {pilihan_bulan}")
    fig_ytd = go.Figure()
    ...
    st.plotly_chart(fig_ytd, use_container_width=True, key="chart_rjp_bgt_kebun_ytd")
...
with col_t1:
    st.markdown(f"##### 📋 Data RJP Per Kebun - {pilihan_bulan}")
    ...
    site_mtd_bgt = df_mtd[col_bgt].sum() / total_pokok_mtd
    ...
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Jg/Pkk)', 'Budget (Jg/Pkk)', 'Gap (Jg/Pkk)', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Jg/Pkk)': '{:,.2f}', 'Budget (Jg/Pkk)': '{:,.2f}', 'Gap (Jg/Pkk)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Jg/Pkk)']).map(style_budget_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_rjp_bgt_kebun_mtd")
...
with col_t2:
    ...
    site_ytd_bgt = df_ytd[col_bgt].sum() / total_pokok_ytd
...
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_rjp_perf_afd")
...
if not df_m_afd.empty:
    ...
    with col_ga1:
        ...
        st.plotly_chart(fig_amtd, use_container_width=True, key="chart_rjp_bgt_afd_mtd")

    with col_ga2:
        st.markdown(f"##### 📊 RJP Per Afdeling ({kebun_terpilih}) - s.d {pilihan_bulan}")
        fig_aytd = go.Figure()
        ...
        st.plotly_chart(fig_aytd, use_container_width=True, key="chart_rjp_bgt_afd_ytd")
    ...
    with col_ta1:
        ...
        df_ta_mtd.columns = ['No', 'Afdeling', 'Aktual (Jg/Pkk)', 'Budget (Jg/Pkk)', 'Gap (Jg/Pkk)', 'Var (%)']
        st.dataframe(df_ta_mtd.style.format({'Aktual (Jg/Pkk)': '{:,.2f}', 'Budget (Jg/Pkk)': '{:,.2f}', 'Gap (Jg/Pkk)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Jg/Pkk)']).map(style_budget_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_rjp_bgt_afd_mtd")
    ...
    with col_ta2:
        ...
        df_ta_ytd.columns = ['No', 'Afdeling', 'Aktual (Jg/Pkk)', 'Budget (Jg/Pkk)', 'Gap (Jg/Pkk)', 'Var (%)']
        st.dataframe(df_ta_ytd.style.format({'Aktual (Jg/Pkk)': '{:,.2f}', 'Budget (Jg/Pkk)': '{:,.2f}', 'Gap (Jg/Pkk)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Jg/Pkk)']).map(style_budget_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_rjp_bgt_afd_ytd")
else:
