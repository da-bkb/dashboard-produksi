...
# --- 1. OTOMATISASI FILTER TIMEFRAME (MTD vs AKUMULASI YTD) ---
URUTAN_BULAN_STD = ['JAN', 'FEB', 'MAR', 'APR', 'MEI', 'JUN', 'JUL', 'AGS', 'SEP', 'OKT', 'NOV', 'DES']

if pilihan_bulan == "CAWU I":
    bulan_mtd_list = ['JAN', 'FEB', 'MAR', 'APR']
    bulan_ytd_list = ['JAN', 'FEB', 'MAR', 'APR']  # s.d Cawu I = Jan - Apr
elif pilihan_bulan == "CAWU II":
    bulan_mtd_list = ['MEI', 'JUN', 'JUL', 'AGS']
    bulan_ytd_list = ['JAN', 'FEB', 'MAR', 'APR', 'MEI', 'JUN', 'JUL', 'AGS']  # s.d Cawu II = Jan - Ags
elif pilihan_bulan == "CAWU III":
    bulan_mtd_list = ['SEP', 'OKT', 'NOV', 'DES']
    bulan_ytd_list = URUTAN_BULAN_STD  # s.d Cawu III = Jan - Des
elif pilihan_bulan == "SEMESTER I":
    bulan_mtd_list = ['JAN', 'FEB', 'MAR', 'APR', 'MEI', 'JUN']
    bulan_ytd_list = ['JAN', 'FEB', 'MAR', 'APR', 'MEI', 'JUN']  # s.d Semester I = Jan - Jun
elif pilihan_bulan == "SEMESTER II":
    bulan_mtd_list = ['JUL', 'AGS', 'SEP', 'OKT', 'NOV', 'DES']
    bulan_ytd_list = URUTAN_BULAN_STD  # s.d Semester II = Jan - Des
else:
    # Jika pilihan bulan tunggal (e.g., 'AGS')
    ...
    if pilihan_bulan_std in URUTAN_BULAN_STD:
        ...
        bulan_ytd_list = URUTAN_BULAN_STD[:idx_bulan + 1]  # Akumulasi Jan s.d Bulan Terpilih
    else:
...
# --- Label Dinamis untuk Judul Grafik & Tabel ---
label_kanan = f"s.d {pilihan_bulan}" if pilihan_bulan not in ["CAWU III", "SEMESTER II"] else "Masing-masing s.d Akhir Tahun"
...
with col_g2:
    st.markdown(f"##### 📊 Yield Per Kebun - Akumulasi ({label_kanan})")
    fig_ytd = go.Figure()
...
with col_t1:
    st.markdown(f"##### 📋 Data Yield Per Kebun - Periode ({pilihan_bulan})")
    ...
    site_mtd_pct = ((site_mtd_akt / site_mtd_bgt * 100) - 100) if site_mtd_bgt > 0 else -100
    
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_mtd_akt, 'Budget': site_mtd_bgt, 'Var': site_mtd_var, 'Pct': site_mtd_pct}])
...
with col_t2:
    st.markdown(f"##### 📋 Data Yield Per Kebun - Akumulasi ({label_kanan})")
    ...
    site_ytd_pct = ((site_ytd_akt / site_ytd_bgt * 100) - 100) if site_ytd_bgt > 0 else -100
    
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_ytd_akt, 'Budget': site_ytd_bgt, 'Var': site_ytd_var, 'Pct': site_ytd_pct}])
...
kebun_terpilled = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_bgt_afd")

df_m_afd = df_mtd[df_mtd['Kebun'] == kebun_terpilled].copy()
df_y_afd = df_ytd[df_ytd['Kebun'] == kebun_terpilled].copy()
...
if not df_m_afd.empty:
    ...
    with col_ga1:
        st.markdown(f"##### 📊 Yield Per Afdeling ({kebun_terpilled}) - Periode ({pilihan_bulan})")
        fig_amtd = go.Figure()
        ...
        fig_amtd.add_trace(go.Scatter(x=[None], y=[None], mode='lines', line=dict(color='#00B050', width=4), name='Budget'))
    ...
    with col_ga2:
        st.markdown(f"##### 📊 Yield Per Afdeling ({kebun_terpilled}) - Akumulasi ({label_kanan})")
        fig_aytd = go.Figure()
        ...
        fig_aytd.add_trace(go.Scatter(x=[None], y=[None], mode='lines', line=dict(color='#00B050', width=4), name='Budget'))
    ...
    with col_ta1:
        st.markdown(f"##### 📋 Data Yield Per Afdeling - Periode ({pilihan_bulan})")
    ...
    with col_ta2:
        st.markdown(f"##### 📋 Data Yield Per Afdeling - Akumulasi ({label_kanan})")
