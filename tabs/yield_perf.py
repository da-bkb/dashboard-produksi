...
df_kebun_bln = df_bln_kebun.groupby("Kebun", as_index=False).agg({"Luas": "sum", "Kg Akt.": "sum", "Kg Bgt.": "sum"})
...
df_kebun_ytd = df_ytd_kebun.groupby("Kebun", as_index=False).agg({"Luas": "sum", "Kg Akt.": "sum", "Kg Bgt.": "sum"})
...
fig.update_layout(title=f"Yield Performa Kebun - Periode {pilihan_bulan}", xaxis_title="Kebun", yaxis_title="Ton / Ha", barmode="group", template="plotly_white")
