...
df_afd_ytd_grp['BJR_Bgt'] = df_afd_ytd_grp['Kg Bgt.'] / df_afd_ytd_grp['Jjg Bgt.']
...
m2.metric("Total Janjang YTD (Jjg)", f"{total_jjg:,.0f}".replace(",", "."))
m3.metric("BJR Realisasi YTD (Kg)", f"{bjr_real:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
...
fig.add_trace(go.Bar(
    ...
    y=df_afd_ytd_grp["BJR_Akt"],
    name="BJR Realisasi",
    marker_color="rgb(50, 171, 96)"
))

fig.update_layout(
    title=f"Grafik BJR per Afdeling - {pilihan_kebun} (YTD s/d {pilihan_bulan})",
...
# --- 3. TABEL DATA ---
df_table = df_afd_ytd_grp[["Afdeling", "Kg Akt.", "Jjg Akt.", "BJR_Akt", "BJR_Pct"]].copy()
...
df_table["BJR_Pct"] = df_table["BJR_Pct"].map('{:,.2f}%'.format)
