...
st.markdown(f"### ⚖️ BJR terhadap Budget")

# --- DETEKSI KOLOM OTOMATIS (Mencegah KeyError) ---
cols = list(df_raw.columns)
col_akt_kg = next((c for c in cols if 'akt' in c.lower() and ('kg' in c.lower() or 'ton' in c.lower() or 'prod' in c.lower())), None)
col_akt_jjg = next((c for c in cols if 'akt' in c.lower() and any(x in c.lower() for x in ['jg', 'jjg', 'jan', 'janjang'])), None)
col_bgt_bjr = next((c for c in cols if ('bgt' in c.lower() or 'budget' in c.lower()) and 'bjr' in c.lower()), None)

# Proteksi jika kolom tidak ditemukan sama sekali
if not col_akt_kg or not col_akt_jjg or not col_bgt_bjr:
    st.error(f"❌ **Error: Kolom BJR / Produksi / Janjang tidak terdeteksi secara otomatis!**")
    st.markdown(f"""
    Aplikasi tidak dapat menemukan kombinasi kolom BJR Aktual / Budget di file Anda.
    * Kolom Kg/Prod ditemukan: `{col_akt_kg}`
    * Kolom Janjang ditemukan: `{col_akt_jjg}`
    * Kolom BJR Budget ditemukan: `{col_bgt_bjr}`
    """)
    st.stop()

# Konversi kolom target ke numeric untuk mengantisipasi data teks/strip agar tidak TypeError
df_raw[col_bgt_bjr] = pd.to_numeric(df_raw[col_bgt_bjr], errors='coerce').fillna(0)
...
df_k_mtd = df_mtd.groupby('Kebun').agg({col_akt_kg: 'sum', col_akt_jjg: 'sum', col_bgt_bjr: 'mean'}).reset_index()
df_k_mtd['Aktual'] = (df_k_mtd[col_akt_kg] / df_k_mtd[col_akt_jjg]).fillna(0)
df_k_mtd['Target'] = df_k_mtd[col_bgt_bjr]
df_k_mtd['Pct'] = (df_k_mtd['Aktual'] / df_k_mtd['Target'] * 100).fillna(0)

df_k_ytd = df_ytd.groupby('Kebun').agg({col_akt_kg: 'sum', col_akt_jjg: 'sum', col_bgt_bjr: 'mean'}).reset_index()
df_k_ytd['Aktual'] = (df_k_ytd[col_akt_kg] / df_k_ytd[col_akt_jjg]).fillna(0)
df_k_ytd['Target'] = df_k_ytd[col_bgt_bjr]
...
with col_t1:
    ...
    site_mtd_bgt = df_mtd[col_bgt_bjr].mean()
...
with col_t2:
    ...
    site_ytd_bgt = df_ytd[col_bgt_bjr].mean()
...
if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({col_akt_kg: 'sum', col_akt_jjg: 'sum', col_bgt_bjr: 'mean'}).reset_index()
    df_a_mtd['Aktual'] = (df_a_mtd[col_akt_kg] / df_a_mtd[col_akt_jjg]).fillna(0)
    df_a_mtd['Target'] = df_a_mtd[col_bgt_bjr]
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({col_akt_kg: 'sum', col_akt_jjg: 'sum', col_bgt_bjr: 'mean'}).reset_index()
    df_a_ytd['Aktual'] = (df_a_ytd[col_akt_kg] / df_a_ytd[col_akt_jjg]).fillna(0)
    df_a_ytd['Target'] = df_a_ytd[col_bgt_bjr]
    ...
    with col_ta1:
        st.markdown(f"##### 📋 Data BJR Per Afdeling - {pilihan_bulan}")
        ...
        df_ta_mtd.columns = ['No', 'Afdeling', 'Aktual (Kg)', 'Budget (Kg)', 'Gap (Kg)', 'Var (%)']
        st.dataframe(df_ta_mtd.style.format({'Aktual (Kg)': '{:,.2f}', 'Budget (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_bgt_afd_mtd")

    with col_ta2:
        st.markdown(f"##### 📋 Data BJR Per Afdeling - s.d {pilihan_bulan}")
        ...
        df_ta_ytd.columns = ['No', 'Afdeling', 'Aktual (Kg)', 'Budget (Kg)', 'Gap (Kg)', 'Var (%)']
        st.dataframe(df_ta_ytd.style.format({'Aktual (Kg)': '{:,.2f}', 'Budget (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_bgt_afd_ytd")
else:
