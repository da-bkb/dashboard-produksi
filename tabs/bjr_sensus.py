...
st.markdown(f"### 🎯 BJR terhadap Sensus")
...
col_sns_bjr = next((c for c in cols if ('sns' in c.lower() or 'sensus' in c.lower()) and 'bjr' in c.lower()), None)

# Proteksi jika kolom tidak ditemukan sama sekali
if not col_akt_kg or not col_akt_jjg or not col_sns_bjr:
    st.error(f"❌ **Error: Kolom BJR / Produksi / Janjang Sensus tidak terdeteksi secara otomatis!**")
    st.markdown(f"""
    Aplikasi tidak dapat menemukan kombinasi kolom BJR Aktual / Sensus di file Anda.
    * Kolom Kg/Prod ditemukan: `{col_akt_kg}`
    * Kolom Janjang ditemukan: `{col_akt_jjg}`
    * Kolom BJR Sensus ditemukan: `{col_sns_bjr}`
    """)
    st.stop()
...
with col_t1:
    ...
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Kg)', 'Sensus (Kg)', 'Gap (Kg)', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Kg)': '{:,.2f}', 'Sensus (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_sns_kebun_mtd")
...
with col_t2:
    ...
    df_final_ytd.columns = ['No', 'Kebun', 'Aktual (Kg)', 'Sensus (Kg)', 'Gap (Kg)', 'Var (%)']
    st.dataframe(df_final_ytd.style.format({'Aktual (Kg)': '{:,.2f}', 'Sensus (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_sns_kebun_ytd")
...
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_bjr_sns_afd")
