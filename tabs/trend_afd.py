...
# --- 2. DETEKSI KOLOM SECARA OTOMATIS (Mencegah KeyError Sensus/Budget) ---
...
# --- 4. DROP DOWN BERTINGKAT (KEBUN -> AFDELING) ---
c_sel_1, c_sel_2 = st.columns(2)

with c_sel_1:
    list_kebun = sorted(df_raw["Kebun"].unique())
    pilihan_kebun = st.selectbox("1. Pilih Kebun:", list_kebun, key="sel_kebun_trend_afd")
...
with c_sel_2:
    ...
    pilihan_afdeling = st.selectbox("2. Pilih Afdeling:", list_afdeling, key="sel_afd_trend_afd")

# Filter data akhir berdasarkan Afdeling terpilih
df_afd = df_filter_kebun[df_filter_kebun["Afdeling"] == pilihan_afdeling].copy()

# --- 5. PROSES AGREGASI DATA BULANAN ---
# Ambil data luas dan pokok untuk afdeling bersangkutan (karena afdeling konstan per bulan, ambil first)
df_luas_total = df_afd.groupby('Bulan').agg({'Luas': 'first', 'Pokok': 'first'}).reset_index()

# Agregasi kolom utama produksi
agg_dict = {COL_KG_AKT: 'sum', COL_JAN_AKT: 'sum'}
if COL_KG_BGT: agg_dict[COL_KG_BGT] = 'sum'
if COL_JAN_BGT: agg_dict[COL_JAN_BGT] = 'sum'

df_main = df_afd.groupby('Bulan').agg(agg_dict).reset_index()
...
for idx, row in df_trend.iterrows():
    ...
    # Hanya memunculkan persentase dan panah jika data aktual sudah terisi (> 0)
    if row["Yield Akt."] > 0:
