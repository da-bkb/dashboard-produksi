...
# --- 0. PROTEKSI TOTAL KONVERSI NUMERIK ---
kolom_angka = ['Luas', 'Kg Akt.', 'Kg Bgt.', 'Kg Sns.']
for col in kolom_angka:
    if col in df_raw.columns:
        # Bersihkan jika tipenya object/string (misal ada titik/koma liar)
...
if df_mtd.empty:
    st.warning(f"Tidak ada data operasional untuk Bulan {pilihan_bulan} di dalam file.")
    st.stop()
...
df_k_mtd['Luas'] = df_k_mtd['Kebun'].map(luas_kebun_mtd).fillna(0)
df_k_mtd['Aktual'] = np.where(df_k_mtd['Luas'] > 0, df_k_mtd['Kg Akt.'] / df_k_mtd['Luas'] / 1000, 0)
df_k_mtd['Target'] = np.where(df_k_mtd['Luas'] > 0, df_k_mtd['Kg Sns.'] / df_k_mtd['Luas'] / 1000, 0)
...
df_k_ytd['Luas'] = df_k_ytd['Kebun'].map(luas_kebun_ytd).fillna(0)
df_k_ytd['Aktual'] = np.where(df_k_ytd['Luas'] > 0, df_k_ytd['Kg Akt.'] / df_k_ytd['Luas'] / 1000, 0)
df_k_ytd['Target'] = np.where(df_k_ytd['Luas'] > 0, df_k_ytd['Kg Sns.'] / df_k_ytd['Luas'] / 1000, 0)
...
with col_t1:
    ...
    site_mtd_akt = df_mtd['Kg Akt.'].sum() / luas_site_mtd / 1000 if luas_site_mtd > 0 else 0
    site_mtd_sns = df_mtd['Kg Sns.'].sum() / luas_site_mtd / 1000 if luas_site_mtd > 0 else 0
    site_mtd_var = site_mtd_akt - site_mtd_sns
    site_mtd_pct = ((site_mtd_akt / site_mtd_sns * 100) - 100) if site_mtd_sns > 0 else 0
    ...
    st.dataframe(df_final_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_var_fill_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True)
...
with col_t2:
    ...
    site_ytd_akt = df_ytd['Kg Akt.'].sum() / luas_site_ytd / 1000 if luas_site_ytd > 0 else 0
    site_ytd_sns = df_ytd['Kg Sns.'].sum() / luas_site_ytd / 1000 if luas_site_ytd > 0 else 0
    site_ytd_var = site_ytd_akt - site_ytd_sns
    site_ytd_pct = ((site_ytd_akt / site_ytd_sns * 100) - 100) if site_ytd_sns > 0 else 0
    ...
    st.dataframe(df_final_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_var_fill_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True)

# --- 5. SUB DETAIL PER AFDELING ---
st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")

list_kebun = sorted(df_raw['Kebun'].dropna().unique()) if 'Kebun' in df_raw.columns else []
if list_kebun:
    ...
    if not df_m_afd.empty:
        ...
        df_a_mtd['Luas'] = df_a_mtd['Afdeling'].map(luas_afd_mtd).fillna(0)
        df_a_mtd['Aktual'] = np.where(df_a_mtd['Luas'] > 0, df_a_mtd['Kg Akt.'] / df_a_mtd['Luas'] / 1000, 0)
        df_a_mtd['Target'] = np.where(df_a_mtd['Luas'] > 0, df_a_mtd['Kg Sns.'] / df_a_mtd['Luas'] / 1000, 0)
        ...
        df_a_ytd['Luas'] = df_a_ytd['Afdeling'].map(luas_afd_ytd).fillna(0)
        df_a_ytd['Aktual'] = np.where(df_a_ytd['Luas'] > 0, df_a_ytd['Kg Akt.'] / df_a_ytd['Luas'] / 1000, 0)
        df_a_ytd['Target'] = np.where(df_a_ytd['Luas'] > 0, df_a_ytd['Kg Sns.'] / df_a_ytd['Luas'] / 1000, 0)
        ...
        with col_ga1:
            ...
            fig_amtd = go.Figure()
            fig_amtd.add_trace(go.Bar(x=df_a_mtd["Afdeling"], y=df_a_mtd["Aktual"], name="Aktual", marker_color="#28348A", width=0.35, text=[f"{p:,.1f}%" for p in df_a_mtd["Pct"]], textposition="inside", insidetextanchor="start"))
            ...
            fig_amtd.update_layout(template="plotly_white", yaxis_title="Ton/Ha", margin=dict(l=20, r=20, t=20, b=20))
            st.plotly_chart(fig_amtd, use_container_width=True)

        with col_ga2:
            st.markdown(f"##### 📊 Yield Per Afdeling ({kebun_terpilih}) - s.d Bulan Ini")
            fig_aytd = go.Figure()
            fig_aytd.add_trace(go.Bar(x=df_a_ytd["Afdeling"], y=df_a_ytd["Aktual"], name="Aktual", marker_color="#28348A", width=0.35, text=[f"{p:,.1f}%" for p in df_a_ytd["Pct"]], textposition="inside", insidetextanchor="start"))
            ...
            fig_aytd.update_layout(template="plotly_white", yaxis_title="Ton/Ha", margin=dict(l=20, r=20, t=20, b=20))
        ...
        with col_ta1:
            ...
            st.dataframe(df_ta_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_var_fill_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True)
        ...
        with col_ta2:
            ...
            st.dataframe(df_ta_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_var_fill_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True)