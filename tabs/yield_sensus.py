...
for col, df, title, k in [(col_t1, df_k_mtd, "Bulan Ini", "table_y_sns_mtd"), (col_t2, df_k_ytd, "s.d Bulan Ini", "table_y_sns_ytd")]:
    with col:
...
kb = st.selectbox("Pilih Kebun:", sorted(df_raw['Kebun'].unique()), key="sb_y_sns_afd")
df_a = df_mtd[df_mtd['Kebun'] == kb].groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Kg Sns.': 'sum', 'Luas': 'sum'}).reset_index()
df_a['Aktual'], df_a['Target'] = df_a['Kg Akt.']/df_a['Luas']/1000, df_a['Kg Sns.']/df_a['Luas']/1000
...
st.dataframe(df_a.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'})
             .map(style_sensus_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_y_sns_afd")