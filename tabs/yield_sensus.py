...
def calc_yield(df):
    g = df.groupby('Kebun').agg({'Kg Akt.': 'sum', 'Kg Sns.': 'sum', 'Luas': 'sum'}).reset_index()
    g['Aktual'] = g['Kg Akt.'] / g['Luas'] / 1000
    g['Target'] = g['Kg Sns.'] / g['Luas'] / 1000
...
for col, df, title, k in [(col_t1, calc_yield(df_mtd), "Bulan Ini", "t_sns_mtd"), (col_t2, calc_yield(df_ytd), "s.d Bulan Ini", "t_sns_ytd")]:
    with col:
        ...
        f.columns = ['No', 'Kebun', 'Aktual (Ton/Ha)', 'Sensus (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
        st.dataframe(f.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'})
                     .map(style_var_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True, key=k)

# --- 3. DETAIL AFDELING ---
st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")
kb = st.selectbox("Pilih Kebun:", sorted(df_raw['Kebun'].dropna().unique()), key="sb_sns_afd")
df_a = df_mtd[df_mtd['Kebun'] == kb].groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Kg Sns.': 'sum', 'Luas': 'sum'}).reset_index()
df_a['Aktual'] = df_a['Kg Akt.'] / df_a['Luas'] / 1000
df_a['Target'] = df_a['Kg Sns.'] / df_a['Luas'] / 1000
df_a['Var'] = df_a['Aktual'] - df_a['Target']
df_a['Pct'] = (df_a['Aktual'] / df_a['Target'] * 100) - 100
df_a.insert(0, 'No', range(1, len(df_a) + 1))
df_a.columns = ['No', 'Afdeling', 'Aktual (Ton/Ha)', 'Sensus (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
st.dataframe(df_a.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'})
             .map(style_var_koreksi, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_sns_afd")