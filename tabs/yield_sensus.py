...
for col, df, title, k in [(col1, df_k_mtd, "Bulan Ini", "sns_chart_mtd"), (col2, df_k_ytd, "s.d Bulan Ini", "sns_chart_ytd")]:
    with col:
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df["Kebun"], y=df["Aktual"], name="Aktual", marker_color="#28348A"))
        fig.add_trace(go.Scatter(x=df["Kebun"], y=df["Target"], mode='lines', name='Sensus', line=dict(color='#00B050', width=3)))
        st.plotly_chart(fig, use_container_width=True, key=k)

# --- 5. TABEL PER AFDELING ---
st.markdown("### 🔎 Detail per Afdeling")
kebun_terpilih = st.selectbox("Pilih Kebun:", sorted(df_raw['Kebun'].unique()), key="sb_sns_afd")
df_a = df_mtd[df_mtd['Kebun'] == kebun_terpilih].groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Kg Sns.': 'sum', 'Luas': 'sum'}).reset_index()
...
st.dataframe(df_a[['Afdeling', 'Aktual', 'Target', 'Var', 'Pct']].style
             ...
             .map(style_sensus_var_fill, subset=['Pct']), use_container_width=True, key="table_sns_afd")
