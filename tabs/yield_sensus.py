...
data_plot = [(col_g1, df_k_mtd, "Bulan Ini", "mtd"), (col_g2, df_k_ytd, "s.d Bulan Ini", "ytd")]

for col, df, title, suffix in data_plot:
    with col:
        st.markdown(f"##### 📊 Yield Per Kebun - {title}")
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df["Kebun"], y=df["Aktual"], name="Aktual", marker_color="#28348A", width=0.35, 
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"yield_sns_chart_{suffix}")
...
def render_table(col, df_base, title, suffix):
    with col:
        st.markdown(f"##### 📋 {title}")
        df_f = df_base.copy()
        df_f['Var'] = df_f['Aktual'] - df_f['Target']
        tot = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_f['Kg Akt.'].sum()/df_f['Luas'].sum()/1000, 
                             'Target': df_f['Kg Sns.'].sum()/df_f['Luas'].sum()/1000}])
        ...
        st.dataframe(df_final.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Sensus (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'})
                     .map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_var_fill_koreksi, subset=['Var (%)']), 
                     use_container_width=True, hide_index=True, key=f"yield_sns_table_{suffix}")

render_table(col_t1, df_k_mtd, "Data Yield Per Kebun - Bulan Ini", "mtd")
render_table(col_t2, df_k_ytd, "Data Yield Per Kebun - s.d Bulan Ini", "ytd")
...
if not df_m_afd.empty:
    ...
    st.dataframe(df_a_mtd[['Afdeling', 'Aktual', 'Target', 'Var', 'Pct']].style.format({'Var': '{:+,.2f}', 'Pct': '{:+,.1f}%'}), use_container_width=True, key="yield_sns_afd_table")
else:
